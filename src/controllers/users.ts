...
export const getUserByEmail = async (c: Context) => {
  ...
  if (!email) return c.json({ error: 'Email manquant' }, 400);
  try {
...
export const createUser = async (c: Context) => {
  try {
    ...
    if (!isNonEmptyString(body.userName)) return c.json({ error: 'Le nom d\'utilisateur ne peut pas être vide' }, 400);
    if (!isValidEmail(body.email)) return c.json({ error: 'Format d\'email invalide' }, 400);
    if (!isStrongPassword(body.password)) return c.json({ error: 'Le mot de passe doit contenir au moins 8 caractères' }, 400);
...
export const updateUser = async (c: Context) => {
  ...
  try {
    ...
    if (existingUserResult.rows.length === 0) return c.json({ error: 'Utilisateur introuvable' }, 404);
