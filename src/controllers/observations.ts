...
export const getObservationsByUserId = async (c: Context) => {
...
export const createObservation = async (c: Context) => {
  try {
    ...
    if (missing.length) return c.json({ error: 'Champs requis manquants', missing }, 400);
    ...
    if (note != null && !isNonEmptyString(note)) return c.json({ error: 'La note doit être une chaîne non vide' }, 400);
