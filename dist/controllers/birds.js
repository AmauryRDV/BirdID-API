...
export const getBirdById = async (c) => {
    ...
    try {
        ...
        if (result.rows.length === 0)
            return c.json({ error: 'Oiseau introuvable' }, 404);
...
export const createBird = async (c) => {
    try {
        ...
        if (!isNonEmptyString(body.birdname))
            return c.json({ error: 'Le nom de l\'oiseau ne peut pas être vide' }, 400);
        if (!isNonEmptyString(body.latinbirdname))
            return c.json({ error: 'Le nom latin de l\'oiseau ne peut pas être vide' }, 400);
        if (!isNonEmptyString(body.media))
            return c.json({ error: 'Le média ne peut pas être vide' }, 400);
        if (!isNonEmptyString(body.funfact))
            return c.json({ error: 'Le funfact ne peut pas être vide' }, 400);
        if (!isNonEmptyString(body.rarity))
            return c.json({ error: 'La rareté ne peut pas être vide' }, 400);
        if (!isNonEmptyString(body.habitat))
            return c.json({ error: 'L\'habitat ne peut pas être vide' }, 400);
...
export const updateBird = async (c) => {
    ...
    try {
        ...
        if (existingBirdResult.rows.length === 0)
            return c.json({ error: 'Oiseau introuvable' }, 404);
        ...
        if (birdname !== undefined && !isNonEmptyString(birdname))
            return c.json({ error: 'Le nom de l\'oiseau ne peut pas être vide' }, 400);
        if (latinbirdname !== undefined && !isNonEmptyString(latinbirdname))
            return c.json({ error: 'Le nom latin de l\'oiseau ne peut pas être vide' }, 400);
        if (media !== undefined && !isNonEmptyString(media))
            return c.json({ error: 'Le média ne peut pas être vide' }, 400);
        if (funfact !== undefined && !isNonEmptyString(funfact))
            return c.json({ error: 'Le funfact ne peut pas être vide' }, 400);
        if (rarity !== undefined && !isNonEmptyString(rarity))
            return c.json({ error: 'La rareté ne peut pas être vide' }, 400);
        if (habitat !== undefined && !isNonEmptyString(habitat))
            return c.json({ error: 'L\'habitat ne peut pas être vide' }, 400);
...
export const deleteBird = async (c) => {
    ...
    try {
        ...
        if (result.rows.length === 0)
            return c.json({ error: 'Oiseau inconnu' }, 404);
