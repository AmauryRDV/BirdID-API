...
export const getAllAchievements = async (c) => {
    try {
...
export const getAchievementById = async (c) => {
    ...
    try {
        ...
        if (result.rows.length === 0)
            return c.json({ error: 'Succès introuvable' }, 404);
...
export const getUserAchievements = async (c) => {
...
export const updateUserAchievement = async (c) => {
    const userId = parseInt(c.req.param('id'), 10);
